---
title: "Establishing Group Equality LAB"
output:
  html_document:
    theme: readable
    df_print: paged
...
Our decision criteria allows for Type I errors (incorrectly identifying difference that don't actually exist) five percent of the time. So let's assume that we are comparing 20 group attributes in our table. Assuming group equivalence, if the probability of a Type I error is **five percent for a single variable** what happens if we test 20 variables at the same time? What is the likelihood that we observe at least one contrast with a p-value below 0.05 when the groups are statistically identical, leading us to incorrectly conclude that the groups are not equivalent? It's a lot more than 0.05 if we perform 20 independent contrasts and use the same decision criteria for rejecting the null that we would use for a single test. 

To use a coin-flip metaphor, if regulators visit a casino to make sure that the coins are fair. They flip the same coin four times to see if they are getting different results. The probability of getting heads all four times is:

(0.5)(0.5)(0.5)(0.5) = 0.0625

So to keep things simple the regulating agency says if this occurs the casino gets fined. Similar to a hypothesis-testing scenario, in 6.25 percent of the visits to casinos that are actually using fair coins they will unjustly receive a fine (a Type I error). The regulator and the casinos agree that it's an appropriate amount of risk because the alternatives to random tests of equipment are expensive and time-consuming (the fair casino is better off paying an erroneous fine every now and then than constantly paying for audits). 
...
Now, if at least four or of ten flips are heads, then the casino gets fined. Did the larger sample size improve their accuracy? 

Obviously not. On average 5 out of 10 coins should turn up head, so a majority of the time there will be at least four heads in ten trials. 

Instead of increasing the accuracy of the test, the regulatory agency has significantly increase the rate of Type I errors!

The problem is that they did not select an level of confidence (0.0625 in the first case) and then determine a process that will fail approximately six percent of the time when the groups are actually equivalent. 

Similarly, if we are now conducting 20 contrasts and each has a Type I error rate of 0.05 then there is a high probability that one out of 20 tests have p-values below 0.05 by random. 
...
So for a concrete example, consider again the table from the Millenium Challenge Villages study. 
...
# use this for more accurate p-values:
